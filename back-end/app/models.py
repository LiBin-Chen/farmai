...
class Permission:
    ...
    FOLLOW:0b00000001 转换为十六进制为0x01
    COMMENT:0b00000010 转换为十六进制为0x02
    WRITE:0b00000100 转换为十六进制为0x04
    ......
    ADMIN:0b10000000 转换为十六进制为0x80
    中间还预留了第4,5,6,7共4位二进制位,以备后续增加操作权限
    """
...
class Role(PaginatedAPIMixin, db.Model):
    ...
    # 当新增用户时,是否当前角色作为默认角色赋予新用户
    default = db.Column(db.Boolean, default=False, index=True)
    # 角色拥有的权限,各操作对应一个二进制位,能执行某项操作的角色,其位会被设为1
    ...
    def insert_roles():
        """
        应用部署时,应该主动执行此函数,添加如下角色
        注意,未登录的用户,可以浏览,但是不能评论或点赞
        shutup:    0b0000 0000 (0x00) 用户被关小黑屋,收回所有权限
        reader:    0b0000 0011 (0x03) 读者,可以关注别人,评论与点赞但不能发表文章
        author:    0b0000 0111 (0x07) 作者,可以关注别人,评论与点赞,发表文章
        administrator:0b1000 0111 (0x87)  超级管理员,拥有所有权限

        以后如果需要添加新角色,或者修改角色的权限,修改roles数组,再运行函数即可
        :return:
        """
        roles = {
            'shutup': ('小黑屋', ()),
    ...
    def get_permissions(self):
        ...
        # 过滤掉没有权限,注意不能用for循环,因为遍历列表是删除元素可能结果并不是你想要的,参考: https://segmentfault.com/a/1190000007214571
        new_p = filter(lambda x: self.has_permission(x[0]), p)
        return ','.join([x[1] for x in new_p])
    ...
    def from_dict(self, data):
        for field in data:
            if field in ['slug', 'name', 'permissions']:
                if field in data:
                    setattr(self, field, data[field])
...
class Task(PaginatedAPIMixin, db.Model):
    ...
    def get_progress(self):
        ...
        return rq_job.meta.get('progress', 0) if rq_job is not None else 10
